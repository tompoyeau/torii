      ...
      <section class="group">
        ...
        <div class="account">
          ...
          <template v-if="steamConnected">
            <p class="hint">
              Bibliothèque synchronisée<template v-if="steamId"> — SteamID <code>{{ steamId }}</code></template>.
            </p>
            <div class="row">
...
.panel {
  ...
  box-shadow: var(--shadow-hero); padding: 24px 26px 20px;
}
.panel-head { display: flex; align-items: center; margin-bottom: 20px; }
.panel-head h2 { font-size: 20px; font-weight: 700; letter-spacing: -0.02em; margin: 0; }
.close {
...
.account.soon { opacity: 0.66; }
.account-top { display: flex; align-items: center; gap: 10px; }
.account-top .dot { width: 9px; height: 9px; border-radius: 50%; }
.account-name { font-weight: 600; font-size: 14.5px; }
.badge {
  margin-left: auto; font-size: 11px; font-family: var(--mono); padding: 3px 9px; border-radius: 99px;
  background: var(--surface-3); color: var(--text-faint);
}
.badge.on { background: color-mix(in srgb, var(--manual) 20%, transparent); color: var(--manual); }
.hint { font-size: 12.5px; line-height: 1.5; color: var(--text-dim); margin: 10px 0; }
.hint code { font-family: var(--mono); color: var(--text); }
.row { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
.row.advanced { margin-top: 8px; }
.row input {
  flex: 1; min-width: 200px; padding: 10px 13px; border-radius: 10px; border: 1px solid var(--border);
  background: var(--surface); color: var(--text); font-size: 13.5px; font-family: var(--mono);
}
...
.advanced-toggle {
  margin-top: 12px; background: none; border: none; color: var(--text-faint);
  font-size: 12px; font-family: var(--mono); padding: 2px 0;
}
.advanced-toggle:hover { color: var(--text-dim); }
.message { font-size: 12.5px; color: var(--accent); margin: 12px 0 0; line-height: 1.4; }
